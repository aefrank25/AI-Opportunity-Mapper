...
// `sendWaitlistDigest` has moved to `./brief-waitlist-digest.server.ts` as a
// plain server-only async function. It is no longer exposed as a server-fn
// RPC — only the auth-gated `/api/public/hooks/waitlist-digest` route can
// invoke it.
